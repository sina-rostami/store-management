...
def get_customers(current_user):
    try:
        ...
        return jsonify(
            {'message': f'An error occurred while getting customers : {e}'}), HTTPStatus.INTERNAL_SERVER_ERROR
...
def add_seller(current_user):
    try:
        ...
        did_success, message = backend.seller_manager.create_seller(request.json)
        if not did_success:
            if message == 'USER_EXISTS':
                return jsonify({'message': message}), HTTPStatus.NOT_ACCEPTABLE
            return jsonify({'message': message}), HTTPStatus.BAD_REQUEST
